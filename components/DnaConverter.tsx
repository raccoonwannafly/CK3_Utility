...
import { convertDnaStringToCode, convertCodeToDnaString, decodeDnaToData, encodeDataToDna, graftDna, VANILLA_GENES, EPE_GENES } from '../services/dnaConverter';
import { DnaData } from '../types';

const DnaConverter: React.FC = () => {
  ...
  // Grafter State
  const [isGrafterOpen, setIsGrafterOpen] = useState(false);
  const [graftOld, setGraftOld] = useState('');
  const [graftTemplate, setGraftTemplate] = useState('');
  const [graftResult, setGraftResult] = useState('');
  ...
  const handleGraft = () => {
      if (!graftOld || !graftTemplate) {
          alert("Please provide both old DNA and a template.");
          return;
      }
      const res = graftDna(graftOld, graftTemplate);
      setGraftResult(res);
  };
    ...
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in p-4">
      <header className="border-b border-stone-700 pb-4 mb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex gap-2">
            <button 
                onClick={() => setIsGrafterOpen(true)}
                className="bg-stone-800 hover:bg-stone-700 text-stone-300 border border-stone-600 px-4 py-1.5 rounded text-xs font-bold transition-colors uppercase"
            >
                🧬 Legacy Grafter
            </button>
      ...
      {isGrafterOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-fade-in">
              <div className="bg-stone-900 border border-ck3-gold max-w-2xl w-full rounded-lg shadow-2xl p-6 relative">
                  <button onClick={() => setIsGrafterOpen(false)} className="absolute top-4 right-4 text-stone-500 hover:text-white text-2xl">&times;</button>
                  <h3 className="text-2xl font-serif text-ck3-gold mb-2">Legacy DNA Grafter</h3>
                  <p className="text-xs text-stone-500 mb-6">Repair old/broken DNA strings by grafting their data onto a fresh template.</p>
                  
                  <div className="space-y-4">
                      <div>
                          <label className="block text-xs uppercase font-bold text-stone-400 mb-1">1. Old / Broken DNA</label>
                          <textarea 
                             className="w-full h-24 bg-black/30 border border-stone-700 rounded p-2 text-xs font-mono text-stone-300 focus:border-ck3-gold outline-none resize-none"
                             value={graftOld}
                             onChange={e => setGraftOld(e.target.value)}
                             placeholder="Paste the problematic Base64 string here..."
                          />
                      </div>
                      <div>
                          <label className="block text-xs uppercase font-bold text-stone-400 mb-1">2. Template DNA (Valid New Character)</label>
                          <textarea 
                             className="w-full h-24 bg-black/30 border border-stone-700 rounded p-2 text-xs font-mono text-stone-300 focus:border-ck3-gold outline-none resize-none"
                             value={graftTemplate}
                             onChange={e => setGraftTemplate(e.target.value)}
                             placeholder="Paste a valid Base64 string from the current game version here..."
                          />
                      </div>
                      
                      <button onClick={handleGraft} className="w-full bg-stone-700 hover:bg-stone-600 text-white font-bold py-3 rounded uppercase transition-colors">
                          Graft & Fix DNA
                      </button>
                      
                      <div>
                          <label className="block text-xs uppercase font-bold text-ck3-gold mb-1">3. Result</label>
                          <textarea 
                             className="w-full h-24 bg-stone-800 border border-stone-600 rounded p-2 text-xs font-mono text-stone-200 outline-none resize-none"
                             value={graftResult}
                             readOnly
                             placeholder="Result will appear here..."
                          />
                          {graftResult && (
                              <button onClick={() => copyToClipboard(graftResult)} className="mt-2 text-xs text-ck3-gold hover:text-white underline">
                                  Copy to Clipboard
                              </button>
                          )}
                      </div>
